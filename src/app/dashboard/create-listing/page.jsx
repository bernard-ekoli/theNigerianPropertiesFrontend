...
export default function CreateListingPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const initializePayment = await fetch(`/api/payment/initialize`, {
        method: 'POST',
        headers: { "Content-Type": "application/json" },
      ...
      popup.resumeTransaction(access_code, {
        onSuccess: async (transaction) => {
          ...
          if (verifyData.success) {
            // 6. Activate Listing
            const activateListingRes = await fetch(`/api/listing/activate-listing/${listingDatares.data.listingId}`, {
              method: "GET",
              credentials: "include"
            });
